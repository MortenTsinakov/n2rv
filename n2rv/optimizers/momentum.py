...
class Momentum(Optimizer):
    ...
    def update(self, layers: list[Layer]) -> None:
        ...
        """
        # If it's the first iteration, initialize the momentum terms for
        # each layer.
        ...
        for layer in layers:
            if layer.trainable:
                self.momentum_terms[layer]["w"] = (
                    self.momentum * self.momentum_terms[layer]["w"]
                    + (1 - self.momentum) * layer.grad_weights
                )
                self.momentum_terms[layer]["b"] = (
                    self.momentum * self.momentum_terms[layer]["b"]
                    + (1 - self.momentum) * layer.grad_bias
                )
                layer.update(
                    self.learning_rate * self.momentum_terms[layer]["w"],
