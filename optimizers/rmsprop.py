...
class RMSProp(Optimizer):
    ...
    def update(self, layers: list[Layer]) -> None:
        ...
        for layer in layers:
            if layer.trainable:
                ...
                layer.update(self.get_change_of_weight_for_layer(layer),
    ...
    def validate_arguments(self, learning_rate: float, beta: float, epsilon: float) -> None:
        """
        ...
        Parameters:
            ...
            epsilon (float): small value to avoid divisioin by zero set on initialization.
        """
    ...
    def validate_learning_rate(self, learning_rate: float) -> None:
        ...
        if not isinstance(learning_rate, float):
            raise TypeError(f"Learning rate must be of type float. Got: {type(learning_rate)}")
        if learning_rate <= 0:
    ...
    def validate_epsilon(self, epsilon: float) -> None:
        ...
        Parameters:
            epsilon (float): small number to avoid division by zero set on initialization.
        """
        if not isinstance(epsilon, float):
            raise TypeError(f"Epsilon must be of type float. Got {type(epsilon)}")
        if epsilon <= 0:
